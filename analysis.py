...
def calculate_velocities(df):
    """Calculates velocities and speeds for each tracked object."""
    df[['center_x', 'center_y']] = (df[['x1', 'y1']] + df[['x2', 'y2']]) / 2
    df = df.sort_values(by=['track_id', 'frame'])
    df['time_delta'] = df.groupby('track_id')[['frame']].diff().fillna(0)
    df['x_delta'] = df.groupby('track_id')[['center_x']].diff().fillna(0)
    df['y_delta'] = df.groupby('track_id')[['center_y']].diff().fillna(0)
    df['velocity_x'] = df['x_delta'] / df['time_delta']
    df['velocity_y'] = df['y_delta'] / df['time_delta']
...
def analyze_possession(df, team_mapping):
    ...
    for frame_num in tqdm(sorted(df['frame'].unique()), desc="Analyzing Possession"):
        ...
        ball_pos = ball_in_frame[['center_x', 'center_y']].iloc[0]
        distances = np.sqrt(((players_in_frame[['center_x', 'center_y']] - ball_pos)**2).sum(axis=1))
...
def calculate_pitch_control(df, team_mapping):
    """Calculates pitch control for each frame using Voronoi diagrams."""
    ...
    for frame_num in tqdm(unique_frames, desc="Calculating Pitch Control"):
        ...
        pitch_control_metrics.append({
            'frame': frame_num,
            'team_0_control_area': team_areas[0],
            'team_1_control_area': team_areas[1]
        })

    return pd.DataFrame(pitch_control_metrics)

def calculate_pressure_metrics(df, possession_df):
    """Calculates pressure on the ball carrier."""
    pressure_metrics = []
    pressure_radius = 100 # 10 meters in pixels
    ...
    for frame_num in tqdm(sorted(frames_with_possession['frame'].unique()), desc="Calculating Pressure"):
        ...
        possessor_pos = possessor_data[['center_x', 'center_y']].iloc[0]
        ...
        distances = np.sqrt(((opponents[['center_x', 'center_y']] - possessor_pos)**2).sum(axis=1))
        ...
        if num_pressuring > 0:
            ...
            for _, opponent in pressuring_opponents.iterrows():
                vec_to_possessor = possessor_pos - opponent[['center_x', 'center_y']]
                norm_vec = vec_to_possessor / np.linalg.norm(vec_to_possessor)
                opponent_vel = opponent[['velocity_x', 'velocity_y']]
...
def calculate_line_breaking_passes(df, possession_df):
    """Identifies line-breaking passes."""
    ...
    for _, pass_event in tqdm(passes.iterrows(), total=passes.shape[0], desc="Finding Line-Breaking Passes"):
        ...
        passer_pos = passer_data[['center_x', 'center_y']].iloc[0]
        receiver_pos = receiver_data[['center_x', 'center_y']].iloc[0]
        ...
        min_x, max_x = min(passer_pos['center_x'], receiver_pos['center_x']), max(passer_pos['center_x'], receiver_pos['center_x'])
        min_y, max_y = min(passer_pos['center_y'], receiver_pos['center_y']), max(passer_pos['center_y'], receiver_pos['center_y'])

        opponents_in_corridor = opponents[
            (opponents['center_x'] > min_x) & (opponents['center_x'] < max_x) &
            (opponents['center_y'] > min_y) & (opponents['center_y'] < max_y)
        ]
...
def calculate_xg(df, possession_df):
    """Calculates Expected Goals (xG) for shots using a heuristic model."""
    # Define goal area (assuming goal is on the right side of the pitch)
    pitch_length = 1050
    goal_y_center = 680 / 2
    goal_width_pixels = 73.2 # 7.32m * 10px/m
    goal_posts = [(pitch_length, goal_y_center - goal_width_pixels/2), (pitch_length, goal_y_center + goal_width_pixels/2)]

    shots = []
    ball_df = df[df['class_name'] == 'ball'].copy()
    ball_df['speed_change'] = ball_df.groupby('track_id')[['speed']].diff().fillna(0)
    
    # Heuristic for shot detection: large, positive change in ball speed near a player
    shot_speed_threshold = 50 # A sharp increase in speed
    ...
    for _, shot_event in tqdm(potential_shots.iterrows(), total=potential_shots.shape[0], desc="Calculating xG"):
        ...
        # Find the player who shot the ball (closest player at that frame)
        shot_pos = shot_event[['center_x', 'center_y']]
        players_in_frame = df_frame[df_frame['class_name'].isin(['player', 'goalkeeper'])]
        if players_in_frame.empty: continue

        distances = np.sqrt(((players_in_frame[['center_x', 'center_y']] - shot_pos)**2).sum(axis=1))
        shooter_id = players_in_frame.iloc[distances.idxmin()]['track_id']
        shooter_team = players_in_frame.iloc[distances.idxmin()]['team_id']

        # Calculate features
        distance_to_goal = np.linalg.norm(shot_pos - np.array([pitch_length, goal_y_center]))
        
        v1 = goal_posts[0] - shot_pos
        v2 = goal_posts[1] - shot_pos
        angle = np.arccos(np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2)))
        angle_degrees = np.degrees(angle)

        # Heuristic xG model (coefficients are illustrative)
        # A simple logistic-style function of distance and angle
        log_odds = 1.5 - (0.02 * distance_to_goal) + (0.05 * angle_degrees)
